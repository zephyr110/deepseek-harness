...
- **Preload** (`src/preload/index.ts`): a `contextBridge` surface named `window.dsh` with exactly four methods — `fetchRequest`, `onStream`, `loadBundle`, `bootManifest` — and no raw `ipcRenderer` exposure.
- **Renderer** (`src/renderer/main.tsx`, `transport.ts`): requests the boot manifest over IPC, installs `window.__DSH_DESKTOP__`, and runs the shared `AppWebEntry` shell from `dsh-client-web` with `loadBundle` implemented over IPC.
